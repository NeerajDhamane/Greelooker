...
const Register = () => {
  ...
  // ── STEP 1: Save details → move to OTP screen ───────────────
  ...
  const verifyOTP = async () => {
    ...
    try {
      const res = await api.post('/auth/register', {
        ...
        password: code,          // OTP used as password for now
      })
      ...
      // Phone already registered → fall back to mock for demo
      if (err.response?.status === 400 && msg.includes('already')) {
        toast.success(`Welcome, ${formData.name}! 🌱`)
        login({ name: formData.name, phone: formData.phone, token: 'mock-token' })
        navigate('/dashboard')
        return
      }
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', padding:'0 16px', background:'var(--bg)' }}>
      ...
      <div style={{
        ...
        {/* ── SCREEN 1 — Details ── */}
        {screen === 'details' && (
          <form onSubmit={handleSubmit(onDetailsSubmit)} style={{ display:'flex', flexDirection:'column', gap:'20px' }}>
        ...
        {screen === 'otp' && (
          <div style={{ display:'flex', flexDirection:'column', gap:'28px' }}>
            ...
            <div style={{ textAlign:'center', fontSize:'14px', color:'var(--text-muted)' }}>
              Didn't receive it?{' '}
              <span onClick={() => setOtp(['','','','','',''])} style={{ color:'var(--accent)', fontWeight:'600', cursor:'pointer' }}>
                Resend OTP
              </span>
            </div>
