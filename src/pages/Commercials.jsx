...
const plans = [
  { name:'Starter',    price:'₹2,999', period:'/month',  desc:'Perfect for small cafés, boutiques or startups just getting started with greening.',                         features:['Up to 10 plants','Monthly maintenance visit','Basic plant selection','Email support','Replacement guarantee'],                                                           cta:'Get Started',  highlight:false },
  { name:'Growth',     price:'₹6,999', period:'/month',  desc:'Our most popular plan for growing businesses who want a serious green transformation.',                        features:['Up to 30 plants','Biweekly maintenance','Custom design consultation','Priority WhatsApp support','Seasonal rotation','Pest & disease cover'],                        cta:'Most Popular', highlight:true  },
  { name:'Enterprise', price:'Custom', period:'pricing', desc:'For large offices, hotel chains and commercial properties with custom requirements.',                           features:['Unlimited plants','Weekly maintenance','Dedicated plant manager','24/7 support','Green wall installations','Staff plant care training'],                            cta:'Talk to Us',   highlight:false },
]
...
const QuoteModal = ({ onClose, isMobile }) => {
  ...
  return (
    <>
      ...
      <div style={{ position:'fixed', top:'50%', left:'50%', transform:'translate(-50%,-50%)', width: isMobile ? '95vw' : '520px', maxWidth:'95vw', maxHeight:'90vh', overflowY:'auto', background:'#fff', borderRadius:'24px', zIndex:100, boxShadow:'0 24px 64px rgba(26,46,26,0.2)', padding: isMobile ? '24px 20px' : '40px' }}>
        {submitted ? (
          <div style={{ textAlign:'center', padding:'24px 0' }}>
            ...
            <p style={{ fontSize:'14px', color:'var(--text-muted)', lineHeight:'1.7', marginBottom:'28px' }}>Thanks {form.name}! Our team will reach out within 24 hours.</p>
            <button onClick={onClose} style={{ padding:'12px 32px', borderRadius:'50px', border:'none', background:'var(--text-hero)', color:'#fff', fontSize:'14px', fontWeight:'600', cursor:'pointer', fontFamily:"'DM Sans',sans-serif" }}>Close</button>
          </div>
        ) : (
          <>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'24px' }}>
              <div>
                <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:'20px', color:'var(--text-hero)', marginBottom:'4px' }}>Get a free quote</h2>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'14px' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap:'12px' }}>
                <div>
                  ...
                  <input value={form.size} onChange={e => set('size', e.target.value)} placeholder="e.g. 800" type="number"
                    style={{ width:'100%', padding:'10px 14px', borderRadius:'12px', border:'1.5px solid var(--border)', fontSize:'13px', fontFamily:"'DM Sans',sans-serif", outline:'none', boxSizing:'border-box' }} />
                </div>
                <div>
  ...
  return (
    <div style={{ background:'var(--bg)', minHeight:'100vh' }}>
      ...
      <div style={{ background:'var(--text-hero)', minHeight:'100vh', display:'flex', flexDirection:'column', position:'relative', overflow:'hidden' }}>
        ...
        {/* Navbar */}
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding: isMobile ? '16px 20px' : '24px 60px', position:'relative', zIndex:10 }}>
          <span style={{ fontFamily:"'Playfair Display',serif", fontSize: isMobile ? '18px' : '22px', fontWeight:'700', color:'#fff' }}>
            Gree<span style={{ color:'var(--soft-leaf)' }}>Looker</span>
            {!isMobile && <span style={{ fontSize:'11px', fontWeight:'500', color:'rgba(255,255,255,0.4)', marginLeft:'12px', letterSpacing:'0.1em', textTransform:'uppercase' }}>for Business</span>}
          </span>
          <div style={{ display:'flex', gap:'8px', alignItems:'center' }}>
            <a href="/" style={{ fontSize:'13px', fontWeight:'500', color:'rgba(255,255,255,0.6)', textDecoration:'none' }}>← Home</a>
            <button onClick={() => setQuoteOpen(true)} style={{ padding: isMobile ? '8px 16px' : '10px 24px', borderRadius:'50px', border:'1.5px solid rgba(255,255,255,0.2)', background:'transparent', color:'#fff', fontSize:'13px', fontWeight:'600', cursor:'pointer', fontFamily:"'DM Sans',sans-serif" }}>
              Get Quote
            </button>
          </div>
        </div>
        ...
        {/* Hero content */}
        <div style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', flexDirection:'column', textAlign:'center', padding: isMobile ? '40px 20px 60px' : '60px 40px', position:'relative', zIndex:10 }}>
          ...
          <h1 style={{ fontFamily:"'Playfair Display',serif", fontSize: isMobile ? '36px' : '64px', fontWeight:'700', color:'#fff', lineHeight:'1.1', marginBottom:'20px', maxWidth:'780px' }}>
            Spaces that breathe.<br />
            <em style={{ color:'var(--soft-leaf)' }}>Businesses that grow.</em>
          </h1>
          <p style={{ fontSize: isMobile ? '14px' : '18px', color:'rgba(255,255,255,0.6)', lineHeight:'1.7', maxWidth:'560px', marginBottom:'36px' }}>
            We design, install and maintain living green spaces for cafés, offices and hotels across India.
          </p>
          <div style={{ display:'flex', gap:'12px', alignItems:'center', flexWrap:'wrap', justifyContent:'center' }}>
            <button onClick={() => setQuoteOpen(true)} style={{ padding: isMobile ? '13px 28px' : '16px 36px', borderRadius:'50px', border:'none', background:'var(--soft-leaf)', color:'var(--text-hero)', fontSize: isMobile ? '14px' : '16px', fontWeight:'700', cursor:'pointer', fontFamily:"'DM Sans',sans-serif" }}>
              Get a Free Quote →
            </button>
            <a href="#how-it-works" style={{ padding: isMobile ? '13px 28px' : '16px 36px', borderRadius:'50px', border:'1.5px solid rgba(255,255,255,0.2)', color:'rgba(255,255,255,0.8)', fontSize: isMobile ? '13px' : '15px', fontWeight:'600', textDecoration:'none' }}>
              How It Works
            </a>
          </div>

          {/* Stats */}
          <div style={{ display:'flex', gap: isMobile ? '20px' : '40px', marginTop: isMobile ? '40px' : '64px', flexWrap:'wrap', justifyContent:'center' }}>
            {[['500+','Plants installed'],['50+','Business clients'],['4.9★','Average rating'],['0','Plants lost']].map(([n, l]) => (
              <div key={l} style={{ textAlign:'center' }}>
                <div style={{ fontSize: isMobile ? '22px' : '28px', fontWeight:'700', color:'#fff', fontFamily:"'Playfair Display',serif" }}>{n}</div>
      ...
      {/* ── HOW IT WORKS ── */}
      <div id="how-it-works" style={{ padding: isMobile ? '48px 20px' : '80px 60px', background:'#fff' }}>
        <div style={{ textAlign:'center', marginBottom:'40px' }}>
          ...
          <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize: isMobile ? '26px' : '36px', color:'var(--text-hero)' }}>How it works</h2>
        </div>
        <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3,1fr)', gap:'20px', maxWidth:'900px', margin:'0 auto' }}>
          {[
            ...
            <div key={s.step} style={{ textAlign:'center', padding: isMobile ? '24px 20px' : '32px 24px', borderRadius:'24px', border:'1.5px solid var(--border)', background:'var(--surface)' }}>
              <div style={{ fontSize:'11px', fontWeight:'700', letterSpacing:'0.12em', color:'var(--text-muted)', marginBottom:'12px' }}>{s.step}</div>
              <div style={{ fontSize:'32px', marginBottom:'12px' }}>{s.icon}</div>
              <h3 style={{ fontFamily:"'Playfair Display',serif", fontSize:'17px', color:'var(--text-hero)', marginBottom:'8px' }}>{s.title}</h3>
      ...
      {/* ── SEGMENT SHOWCASE ── */}
      <div style={{ padding: isMobile ? '48px 20px' : '80px 60px', background:'var(--bg)' }}>
        <div style={{ textAlign:'center', marginBottom:'32px' }}>
          ...
          <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize: isMobile ? '24px' : '36px', color:'var(--text-hero)' }}>Built for every commercial space</h2>
        </div>

        {/* Segment tabs */}
        <div style={{ display:'flex', justifyContent:'center', gap:'8px', marginBottom:'36px', flexWrap:'wrap' }}>
          {Object.keys(segments).map(s => (
            <button key={s} onClick={() => setActiveSegment(s)} style={{ padding: isMobile ? '8px 16px' : '10px 24px', borderRadius:'50px', border: activeSegment===s ? '1.5px solid var(--accent)' : '1.5px solid var(--border)', background: activeSegment===s ? 'var(--pill)' : '#fff', color: activeSegment===s ? 'var(--accent)' : 'var(--text-muted)', fontSize: isMobile ? '13px' : '14px', fontWeight: activeSegment===s ? '600' : '500', cursor:'pointer', fontFamily:"'DM Sans',sans-serif" }}>
              {s}
            </button>
          ))}
        </div>
        ...
        {(() => {
          ...
          return (
            <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? '24px' : '48px', alignItems:'center', maxWidth:'1100px', margin:'0 auto' }}>
              <div>
                <img src={seg.img} alt={activeSegment} style={{ width:'100%', height: isMobile ? '220px' : '400px', objectFit:'cover', borderRadius:'20px', display:'block' }} onError={e => e.target.style.display='none'} />
              </div>
              <div>
                <h3 style={{ fontFamily:"'Playfair Display',serif", fontSize: isMobile ? '22px' : '28px', color:'var(--text-hero)', marginBottom:'12px', lineHeight:'1.3' }}>{seg.headline}</h3>
                <p style={{ fontSize:'14px', color:'var(--text-muted)', lineHeight:'1.8', marginBottom:'24px' }}>{seg.sub}</p>
                <div style={{ display:'flex', gap:'12px', marginBottom:'24px' }}>
                  {seg.stats.map(st => (
                    <div key={st.l} style={{ textAlign:'center', padding: isMobile ? '12px' : '16px 20px', borderRadius:'16px', background:'var(--text-hero)', flex:1 }}>
                      <div style={{ fontFamily:"'Playfair Display',serif", fontSize: isMobile ? '18px' : '24px', fontWeight:'700', color:'var(--soft-leaf)' }}>{st.n}</div>
                      <div style={{ fontSize:'10px', color:'rgba(255,255,255,0.5)', marginTop:'4px' }}>{st.l}</div>
                    </div>
                  ))}
                </div>
                ...
                <button onClick={() => setQuoteOpen(true)} style={{ padding:'14px 32px', borderRadius:'50px', border:'none', background:'var(--text-hero)', color:'#fff', fontSize:'14px', fontWeight:'700', cursor:'pointer', fontFamily:"'DM Sans',sans-serif", width: isMobile ? '100%' : 'auto' }}>
                  Get a Quote for {isMobile ? 'This' : activeSegment} →
                </button>
              </div>
            </div>
          )
      ...
      {/* ── PRICING ── */}
      <div style={{ padding: isMobile ? '48px 20px' : '80px 60px', background:'#fff' }}>
        <div style={{ textAlign:'center', marginBottom:'40px' }}>
          ...
          <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize: isMobile ? '24px' : '36px', color:'var(--text-hero)', marginBottom:'12px' }}>Simple monthly plans</h2>
          <p style={{ fontSize:'14px', color:'var(--text-muted)' }}>No setup fee. No lock-in. Cancel anytime.</p>
        </div>
        <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3,1fr)', gap:'20px', maxWidth:'960px', margin:'0 auto' }}>
          {plans.map(plan => (
            <div key={plan.name} onClick={() => setActivePlan(plan.name)}
              style={{ padding: isMobile ? '24px 20px' : '32px 28px', borderRadius:'24px', border: plan.highlight ? '2px solid var(--accent)' : '1.5px solid var(--border)', background: plan.highlight ? 'var(--text-hero)' : '#fff', cursor:'pointer', position:'relative', transition:'transform 0.2s', transform: activePlan===plan.name ? 'translateY(-4px)' : 'none' }}>
              {plan.highlight && (
                <span style={{ position:'absolute', top:'-12px', left:'50%', transform:'translateX(-50%)', fontSize:'11px', fontWeight:'700', padding:'4px 16px', borderRadius:'50px', background:'var(--accent)', color:'#fff', whiteSpace:'nowrap' }}>Most Popular</span>
              )}
              ...
              <p style={{ fontSize:'12px', color: plan.highlight ? 'rgba(255,255,255,0.5)' : 'var(--text-muted)', lineHeight:'1.6', marginBottom:'20px' }}>{plan.desc}</p>
              <div style={{ display:'flex', flexDirection:'column', gap:'8px', marginBottom:'24px' }}>
                {plan.features.map(f => (
                  <div key={f} style={{ display:'flex', alignItems:'center', gap:'8px', fontSize:'13px', color: plan.highlight ? 'rgba(255,255,255,0.75)' : 'var(--text-body)' }}>
                    <span style={{ color: plan.highlight ? 'var(--soft-leaf)' : 'var(--accent)', fontWeight:'700', flexShrink:0 }}>✓</span>{f}
                  </div>
                ))}
              </div>
      ...
      <div style={{ padding: isMobile ? '48px 20px' : '80px 60px', background:'var(--bg)' }}>
        <div style={{ textAlign:'center', marginBottom:'40px' }}>
          ...
          <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize: isMobile ? '24px' : '36px', color:'var(--text-hero)' }}>Real spaces. Real results.</h2>
        </div>
        <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3,1fr)', gap:'20px', maxWidth:'1000px', margin:'0 auto' }}>
          {testimonials.map(t => (
            <div key={t.name} style={{ padding: isMobile ? '20px' : '28px', borderRadius:'24px', background:'#fff', border:'1.5px solid var(--border)' }}>
              <div style={{ fontSize:'24px', color:'var(--accent)', marginBottom:'12px', fontFamily:'serif' }}>"</div>
              <p style={{ fontSize:'13px', color:'var(--text-body)', lineHeight:'1.8', marginBottom:'16px', fontStyle:'italic' }}>{t.quote}</p>
      ...
      {quoteOpen && <QuoteModal onClose={() => setQuoteOpen(false)} isMobile={isMobile} />}
    </div>
  )
}
