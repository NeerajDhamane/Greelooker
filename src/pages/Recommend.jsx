...
const Recommend = () => {
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'28px' }}>
            ...
            <div data-err={errors.room ? 'true' : undefined}>
              <Question label="🏠 What type of room is it?" error={errors.room} errorMsg="Please select a room type">
                <Pills isMobile={isMobile} options={['Bedroom','Living Room','Balcony','Office / Study','Kitchen','Bathroom']} selected={answers.room} onSelect={v => pick('room', v)} />
              </Question>
            </div>
            <Divider />
            ...
            <div data-err={errors.floor ? 'true' : undefined}>
              <Question label="🏢 Which floor are you on?" sub="Higher floors get more wind and direct sunlight — affects plant survival" error={errors.floor} errorMsg="Please select your floor">
                <select value={answers.floor} onChange={e => { setAnswers(p => ({...p, floor:e.target.value})); setErrors(p => ({...p, floor:false})) }}
                  style={{ ...inputBase(errors.floor), width: isMobile ? '100%' : '240px', cursor:'pointer' }}>
            ...
            <div data-err={errors.sun ? 'true' : undefined}>
              <Question label="☀️ How much sunlight does the space get?" error={errors.sun} errorMsg="Please select sunlight level">
                <Pills isMobile={isMobile} options={['🌑 Low — barely any light','🌤️ Medium — indirect light','🌞 Bright — lots of indirect','☀️ Direct — full sun most of day']} selected={answers.sun} onSelect={v => pick('sun', v)} />
              </Question>
            </div>
            <Divider />
            ...
            <div data-err={errors.size ? 'true' : undefined}>
              <Question label="📐 How big is the space?" error={errors.size} errorMsg="Please select space size">
                <Pills isMobile={isMobile} options={['Small (under 100 sq ft)','Medium (100–300 sq ft)','Large (300+ sq ft)']} selected={answers.size} onSelect={v => pick('size', v)} />
              </Question>
            </div>
            <Divider />
            ...
            <div data-err={errors.area ? 'true' : undefined}>
              <Question label="📍 Where in the room do you want the plant?" error={errors.area} errorMsg="Please select placement">
                <Pills isMobile={isMobile} options={['🪟 Near the window','🌿 Corner of the room','⬛ Centre of the room','🪴 Balcony edge / railing','🖥️ On my desk','🛁 Bathroom shelf']} selected={answers.area} onSelect={v => pick('area', v)} />
              </Question>
            </div>
            <Divider />
            ...
            <div data-err={errors.city ? 'true' : undefined}>
              <Question label="🌆 Which city are you in?" sub="Climate affects which plants survive and thrive" error={errors.city} errorMsg="Please select your city">
                <Pills isMobile={isMobile} options={['🌊 Mumbai','🌬️ Delhi','🌿 Bangalore','🌶️ Chennai','🌧️ Kolkata','🏔️ Pune','Other']} selected={answers.city} onSelect={v => pick('city', v)} />
              </Question>
            </div>
            <Divider />
  ...
  return (
    <div style={{ display:'flex', height:'100vh', overflow:'hidden', background:'var(--bg)' }}>
      <Sidebar />
      <div style={{ flex:1, overflowY:'auto' }}>
        <div style={{ maxWidth:'960px', margin:'0 auto', padding: isMobile ? '20px 16px 100px' : '48px' }}>

          <button onClick={() => { setScreen('quiz'); window.scrollTo(0,0) }}
            style={{ display:'flex', alignItems:'center', gap:'6px', fontSize:'13px', fontWeight:'600', color:'var(--text-muted)', background:'none', border:'none', cursor:'pointer', fontFamily:"'DM Sans',sans-serif", marginBottom:'24px', padding:0 }}>
            ← Edit answers
          </button>
